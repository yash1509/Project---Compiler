...
import bytecodes
from objects import YashBoolean, YashNumber
...
class Number(Node):
    ...
    def compile(self, ctx):
        ctx.emit(bytecodes.LOAD_CONST, ctx.new_const(YashNumber(self.value)))
...
class Boolean(Node):
    ...
    def compile(self, ctx):
        ctx.emit(bytecodes.LOAD_CONST, ctx.new_const(YashBoolean(self.value)))
